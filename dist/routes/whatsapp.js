...
async function toolSendRenewalNotice(tenantName, increasePercent) {
    ...
    if (!tenant.email)
        return `No email address on file for ${tenant.full_name}.`;
...
async function toolCheckRERA(tenantName, marketRent) {
    const tenants = await (0, data_1.getTenants)();
    const units = await (0, data_1.getUnits)();
    const tenant = tenants.find(t => t.full_name.toLowerCase().includes(tenantName.toLowerCase()));
    if (!tenant)
...
router.post('/incoming', async (req, res) => {
    ...
    if (mediaUrl && isExcel) {
        try {
            ...
            const result = await (0, excel_import_1.importExcelBuffer)(buffer);
            ...
            if (result.buildings > 0)
                lines.push(`🏢 ${result.buildings} building${result.buildings !== 1 ? 's' : ''} added`);
            if (result.units > 0)
                lines.push(`🏠 ${result.units} unit${result.units !== 1 ? 's' : ''} added`);
            if (result.tenants > 0)
    ...
    try {
        await (0, conversation_1.cleanExpiredConversations)();
        const history = await (0, conversation_1.loadHistory)(from);
        ...
        const annotatedCheques = data.cheques
            ...
            status: c.status,
            daysUntilDue: (0, data_1.daysUntil)(c.due_date),
        }))
...
EMAIL: When the landlord asks to send an email to a tenant (in any language), use send_email_to_tenant. Write a professional bilingual email (English + Arabic) as the bodyHtml parameter. Never send WhatsApp to tenants.`;
        ...
        if (firstResponse.stop_reason !== 'tool_use') {
            ...
            await (0, conversation_1.saveHistory)(from, userMessage, answer);
        ...
        for (const block of firstResponse.content) {
            ...
            switch (block.name) {
                ...
                case 'check_rera_increase':
                    result = await toolCheckRERA(input.tenantName, input.marketRent);
                    break;
                case 'send_email':
