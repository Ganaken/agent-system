...
function loadSkills(agentDir) {
  const listPath = path.join(agentDir, 'skills.txt');
  if (!fs.existsSync(listPath)) return '';
  const names = fs.readFileSync(listPath, 'utf8').split('\n').map(s => s.trim()).filter(Boolean);
  const libDir = path.join(agentDir, '..', 'skills');
  const parts = [];
  for (const name of names) {
    const skillPath = path.join(libDir, name, 'SKILL.md');
    if (fs.existsSync(skillPath)) {
      parts.push(`### SKILL: ${name}\n` + fs.readFileSync(skillPath, 'utf8'));
    }
  }
  return parts.length
    ? `\nSKILLS you have been trained with. Apply their guidance:\n\n` + parts.join('\n\n---\n\n')
    : '';
}
...
async function main() {
  ...
  const { data: lessons } = await supabase
    ...
    .eq('active', true);
  const skillBlock = loadSkills(__dirname);
