...
export async function checkChequeReminders(): Promise<void> {
  ...
  for (const cheque of cheques) {
    ...
    if (days <= 7 && !cheque.reminderSent7) {
      await sendEmail(MY_EMAIL(), `🚨 URGENT: Cheque due in ${days} days — ${cheque.tenantName} Unit ${cheque.unit}`, chequeEmail(cheque, days));
      cheque.reminderSent7 = true;
      cheque.reminderSent14 = true;
      cheque.reminderSent30 = true;
      changed = true;
    } else if (days <= 14 && !cheque.reminderSent14) {
      await sendEmail(MY_EMAIL(), `⚠️ Cheque due in ${days} days — ${cheque.tenantName} Unit ${cheque.unit}`, chequeEmail(cheque, days));
      cheque.reminderSent14 = true;
      cheque.reminderSent30 = true;
      changed = true;
    } else if (days <= 30 && !cheque.reminderSent30) {
      await sendEmail(MY_EMAIL(), `📅 Cheque due in ${days} days — ${cheque.tenantName} Unit ${cheque.unit}`, chequeEmail(cheque, days));
      cheque.reminderSent30 = true;
      changed = true;
    }
  }
